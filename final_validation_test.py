...
def test_pipeline_functions():
    ...
    # Test individual functions with smaller batches to prevent overwhelming
    for i in range(0, 300, 25):
        batch = SAMPLE_TEXTS[i:i+25]
        _ = pipelines.process_emoticons(batch)
        _ = pipelines.process_emojis(batch)
        _ = pipelines.process_actions(batch)
        if i % 50 == 0:
            tracker.checkpoint(f"Individual pipelines x{i+25}")
    ...
    for i in range(0, 400, 50):
        ...
        _ = pipelines.process_actions(batch)
        if i % 100 == 0:
            tracker.checkpoint(f"Batch processing x{i+50}")
...
def test_multiprocessing_memory():
    """Test multiprocessing with memory tracking."""
    tracker = MemoryTracker("Multiprocessing Test")
    tracker.start()
    
    def mp_worker_task(text_batch):
        # Import inside the worker (necessary for multiprocessing)
        from texy.direct import extreme_clean_direct, strict_clean_direct
        r1 = extreme_clean_direct(text_batch)
        r2 = strict_clean_direct(text_batch)
        return len(r1) + len(r2)  # Return count instead of data to reduce memory
    
    # Split texts into batches for multiprocessing
    batch_size = 25
    batches = [SAMPLE_TEXTS[i:i+batch_size] for i in range(0, 300, batch_size)]
    
    with ProcessPoolExecutor(max_workers=3) as executor:
        futures = []
        for i, batch in enumerate(batches):
            future = executor.submit(mp_worker_task, batch)
            futures.append(future)
            if i % 3 == 0:
                tracker.checkpoint(f"Submitted {i+1} process tasks")
        ...
        for i, future in enumerate(futures):
            ...
            if i % 3 == 0:
                tracker.checkpoint(f"Completed {i+1} process tasks")
    
    print(f"✅ Multiprocessing completed {total_results} operations")
    return tracker.end()
...
def main():
    ...
    print("\n🧪 Test 4: Multiprocessing Memory")
    print("-" * 40)
    results["multiprocessing"] = test_multiprocessing_memory()
    
    # Test 5: Intensive processing
    print("\n🧪 Test 5: Intensive Processing")
    print("-" * 40)
    ...
    if all_passed:
        ...
        print("\n⚠️  Some tests still show memory growth. Further investigation needed.")
